...
func TestDiskRule(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			diskRule := &DiskRule{Rule: Rule{Compare: test.Compare}, Alert: test.Alert, Warning: test.Warning}

			got, err := diskRule.Analyse(stats.Data{Disk: stats.Memory{Used: test.Disk}})
	...
	t.Run("throw error if no compare character does not match", func(t *testing.T) {
		diskRule := &DiskRule{Rule: Rule{Compare: "foo", Name: "wrong compare"}}

		_, got := diskRule.Analyse(stats.Data{})
		want := "Disk rule 'wrong compare': The compare character does not match neither '>', '<', '=' or '!='"
